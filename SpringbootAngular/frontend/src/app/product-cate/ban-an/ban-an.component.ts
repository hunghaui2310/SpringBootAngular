...
@Component({
  selector: 'app-ban-an',
  templateUrl: './ban-an.component.html',
  styleUrls: ['./ban-an.component.scss']
})
export class BanAnComponent implements OnInit {
  ...
  getDataPro() {
    this.productService.productCateAPI(2).subscribe(
      data => {
  ...
  search() {
    this.searchRequest = new SearchRequest(null, null, 2, this.sortCondition);
